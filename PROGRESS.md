...
### Post-remediation: runtime crash fix (ulid in ESM Lambdas)
Email verification failed in the deployed dev env: `PostConfirmation failed — secure crypto unusable, insecure Math.random not allowed`. Root cause: the `ulid` npm package eagerly runs `ulid = factory()` at import and its PRNG detection uses `require("crypto")`, which doesn't exist in our **ESM-bundled** Lambdas — so *merely importing* `ulid` threw at load. This was latent across the whole platform (session id, tenant id, kb id, message keys) and only surfaced when the post-confirmation trigger was first exercised. Reproduced exactly by esbuild-bundling a handler as ESM. Fixed by replacing the `ulid` dependency with a **self-contained, crypto-backed ULID generator** (`packages/backend/src/lib/ulid.ts`, `randomBytes` via ESM `import`); removed the `ulid` dep. Verified the fix in the ESM-bundle environment (the crypto error is gone; a valid monotonic ULID is produced). Added `ulid.test.ts` (+ property test). 133 tests green.
